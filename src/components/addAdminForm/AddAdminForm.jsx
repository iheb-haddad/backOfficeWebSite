...
function AddAdminForm() {
  ...
  const handleSubProjectChange = (event) => {
    const selectedOption = subProjects.find(
      (subProject) => subProject._id === event.target.value
    );
    if (selectedOption) {
  ...
  const deleteProject = (index) => {
    ...
    const newProjectsSelected = projectsSelected.filter(
      (project, i) => i !== index
    );
    setProjectsSelected(newProjectsSelected);
    const newSubProjectsSelected = subProjectsSelected.filter(
      (project) => project.idProject._id !== projectToDelete._id
    );
  ...
  const handleEnregistrer = () => {
    const hasEmptyFields =
      ...
      (subProjectsSelected.length === 0 && auth?.user?.role === "user");
    ...
    if (!hasEmptyFields) {
      ...
      Axios.post("/auth/register", {
        newAdmin,
        confirmation: formData.confPassword,
      })
        .then((data) => {
    ...
    <div className="addAdmin" style={{ padding: "80px" }}>
      <div className="settingsBody">
        <div className="addAdminForm">
          <div className="addAdminLine">
            ...
            >
              ...
              {projects.filter(
                (prj) =>
                  !projectsSelected.some((prjSelec) => prjSelec._id === prj._id)
              ).length > 0 ? (
                projects
                  .filter(
                    (prj) =>
                      !projectsSelected.some(
                        (prjSelec) => prjSelec._id === prj._id
                      )
                  )
                  .map((project) => (
          ...
          <div className="addAdminLine">
            ...
            >
              ...
              {subProjects.filter(
                (subPrj) =>
                  projectsSelected.some(
                    (projet) => projet._id === subPrj.idProject._id
                  ) &&
                  !subProjectsSelected.some(
                    (projet) => projet._id === subPrj.idProject._id
                  )
              ).length > 0 ? (
                subProjects
                  .filter(
                    (subPrj) =>
                      ...
                      !subProjectsSelected.some(
                        (projet) => projet._id === subPrj._id
                      )
                  )
                  .map((project) => (
                    <option key={project._id} value={project._id}>
                      {project.name + " (" + project.idProject.name + ")"}
                    </option>
                  ))
              ) : (
                <option value="" disabled>
            ...
            <div className="urlBlock">
              {subProjectsSelected.map((prj, index) => (
                <div key={index} className="urlColumn">
                  <FontAwesomeIcon
                    icon={faX}
                    onClick={() => deleteSubProject(index)}
                    style={{
                      cursor: "pointer",
                      fontSize: "7px",
                      position: "absolute",
          ...
          <div className="addAdminLine">
            ...
            <input
              ...
              style={{
                border: emptyFields && !formData.lastName && "1px solid red",
              }}
            />
          </div>
          <div className="addAdminLine">
            ...
            <input
              ...
              placeholder="Saisir titre "
              style={{
                border: emptyFields && !formData.firstName && "1px solid red",
              }}
            />
          </div>
          <div className="addAdminLine">
            ...
            <input
              ...
              placeholder="Saisir titre "
              style={{
                border: emptyFields && !formData.email && "1px solid red",
              }}
            />
          ...
          <div className="addAdminLine">
            ...
            <input
              ...
              placeholder="Saisir titre "
              style={{
                border: emptyFields && !formData.password && "1px solid red",
              }}
            />
          </div>
          <div className="addAdminLine">
            ...
            <input
              ...
              style={{
                border:
                  emptyFields && !formData.confPassword && "1px solid red",
              }}
            />
            <div className="adminErr">
              <p style={{ color: msgErreur2Color }}>
        ...
        <div className="settingsButton">
          ...
          <button className="enregistrer" onClick={handleEnregistrer}>
            Enregistrer
          </button>
        </div>
      </div>

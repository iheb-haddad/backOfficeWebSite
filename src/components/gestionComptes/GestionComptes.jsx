    ...
    >
      <div className="buttonsBox">
        {(auth?.user?.role === "admin" || projects.length > 0) && (
          <Link to="/GestionComptes" onClick={clickUsers}>
            <button className={`back ${btnClicked === "users" && "clicked"}`}>
              Gestion users
            </button>
          </Link>
        )}
        {(auth?.user?.role === "admin" || projects.length > 0) && (
          <Link to="/GestionComptes/ajouterUser" onClick={clickAjouter}>
            <button className={`back ${btnClicked === "ajouter" && "clicked"}`}>
              Ajouter user
            </button>
          </Link>
        )}
        <Link to="/GestionComptes/parametresCompte" onClick={clickParametres}>
          <button
            className={`back ${btnClicked === "parametres" && "clicked"}`}
          >

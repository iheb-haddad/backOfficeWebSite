...
export default function SupportMailConfig() {
  ...
  useEffect(() => {
    ...
    Axios.get("supportEmail/" + formData.subProject).then((res) => {
      const data = res.data;
      if (!data) {
        setFormData({
          id: "",
    ...
    <div className="colorsForm">
      ...
      <div className="configLine">
        ...
        <select
          ...
          onChange={(e) =>
            setFormData({ ...formData, subProject: e.target.value })
          }
          style={{
            border: showError && !formData.subProject && "1px solid red",
          }}
        >
          <option value="" disabled hidden>
            ----
          ...
          {subProjects
            .filter(
              (subProject) => subProject.idProject._id === formData.project
            )
            .map((project) => (
              <option key={project._id} value={project._id}>
                {project.name}

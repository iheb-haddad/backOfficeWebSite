...
function GestionDocuments() {
  ...
  const [docGeneralUrlFieldColor, setDocGeneralUrlFieldColor] =
    useState("white");
    ...
    Axios.get("/configurations")
      .then((response) => {
        if (response.data.length > 0) {
          ...
          setConfSelected(
            response.data.find((conf) => conf.idProject === project) ||
              response.data[0]
          );
    ...
    <div>
      ...
      {(auth?.user?.role === "admin" || projects.length > 0) && (
        <div className="urlBox" style={{ marginBottom: "40px" }}>
          <h4>
            Si vous possedez un url général pour stocker les documentations ,
